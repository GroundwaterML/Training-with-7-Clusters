...
OUTPUT_DIR = os.path.join(ROOT_DIR, '..', 'outputs_spatial_pooling')
...
def compute_spatial_weights(cluster_id):
    file_path = os.path.join(CLUSTERS_DIR, f'cluster_{cluster_id}.xlsx')
    xls = pd.ExcelFile(file_path)
    ...
    weights = np.zeros((W, W))
    for i in range(W):
        for j in range(W):
            dist = haversine_km(lats[i], lons[i], lats[j], lons[j])
            weights[i, j] = np.exp(-(dist ** 2) / (sigma ** 2))
...
class SpatialGRU(nn.Module):
    def __init__(self, seq_input_dim, mem_input_dim, hidden_size, num_layers, dropout, spatial_weights):
        super().__init__()
        ...
        self.spatial_agg = nn.Linear(hidden_size, hidden_size)
    ...
    def forward(self, xs, xm):
        ...
        h_agg = torch.matmul(self.spatial_weights, h_well)
        h_agg = self.spatial_agg(h_agg)
        
        h_combined = torch.cat([h_well, h_agg], dim=2)
...
def train():
    ...
    for seed in SEEDS:
        ...
        for cid, (Xs, Xm, ys, _) in enumerate(all_data):
            ...
            spatial_weights = compute_spatial_weights(cid)
            ...
            model = SpatialGRU(N_SEQ_FEAT, N_MEM_FEAT, HIDDEN_SIZE, N_GRU_LAYERS, DROPOUT, spatial_weights).to(device)
    ...
    results_df.to_csv(os.path.join(OUTPUT_DIR, 'spatial_pooling_results.csv'), index=False)
