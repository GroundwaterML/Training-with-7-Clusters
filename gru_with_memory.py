...
OUTPUT_DIR = os.path.join(ROOT_DIR, '..', 'outputs_gru_memory')
...
class GRUMemory(nn.Module):
    def __init__(self, seq_input_dim, mem_input_dim, hidden_size, num_layers, dropout):
        super().__init__()
        ...
        self.head = nn.Sequential(
            nn.Linear(hidden_size + mem_input_dim, 128),
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, xs, xm):
        ...
        h_last = h[-1]
        h_well = h_last.reshape(B, W, -1)
        
        h_flat = h_well.reshape(B*W, -1)
        xm_flat = xm.reshape(B*W, -1)
        
        h_final = torch.cat([h_flat, xm_flat], dim=1)
...
def train():
    ...
    for seed in SEEDS:
        ...
        for cid, (Xs, Xm, ys, _) in enumerate(all_data):
            ...
            model = GRUMemory(N_SEQ_FEAT, N_MEM_FEAT, HIDDEN_SIZE, N_GRU_LAYERS, DROPOUT).to(device)
    ...
    results_df.to_csv(os.path.join(OUTPUT_DIR, 'gru_memory_results.csv'), index=False)
